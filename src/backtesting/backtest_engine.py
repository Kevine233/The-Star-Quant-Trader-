...
class BacktestEngine:
    ...
    def run_backtest(self, data: Dict[str, pd.DataFrame], strategy_func: Callable, strategy_params: Dict = None) -> Dict:
        ...
        # 预处理数据，创建高效索引
        self._preprocess_data_for_efficient_lookup(data)
    ...
    def _preprocess_data_for_efficient_lookup(self, data: Dict[str, pd.DataFrame]):
        """
        预处理数据，创建高效的日期查找索引
        ...
        logger.info(f"数据预处理完成，已为{len(data)}个品种创建日期索引")
            ...
            if symbol in self._data_cache:
                ...
                if date in date_index:
                    ...
                    if isinstance(row, pd.Series):
                        ...
                        # 如果行不是Series，可能是因为索引出错
                        logger.warning(f"无法获取数据 - 品种: {symbol}, 日期: {date}")
                else:
                    # 日期不在索引中
                    logger.debug(f"日期不在数据范围内 - 品种: {symbol}, 日期: {date}")
            else:
                # 缓存中没有此品种，可能是因为预处理失败
                logger.warning(f"数据未预处理 - 品种: {symbol}")
                ...
                if isinstance(df.index, pd.DatetimeIndex):
                    # 如果索引是日期类型，直接查找
                    if date in df.index:
                        result[symbol] = df.loc[date].to_dict()
                elif 'date' in df.columns:
                    # 如果有日期列，使用布尔索引
                    matches = df[df['date'] == date]
                    if not matches.empty:
                        result[symbol] = matches.iloc[0].to_dict()
