...
from src.data_sources.crypto_data.manager import CryptoDataManager
...
from src.utils import load_market_data, save_market_data, normalize_ohlcv_data
from src.utils.api_config_manager import APIConfigManager
from src.web_interface.api_routes import api_bp
...
class WebController:
    ...
    def _init_components(self):
        ...
        try:
            ...
            self.crypto_data_source = CryptoDataProvider(self.config["data_source"]["crypto"])
            
            # 初始化策略引擎
            self.smart_money_detector = SmartMoneyDetectorV2()
    ...
    def _register_routes(self):
        """注册路由"""
        # 测试路由
        self.app.add_url_rule('/test_template', 'test_template', self.test_template)
        ...
        # 市场监控
        self.app.add_url_rule('/stock', 'stock_market', self.stock_market)
        self.app.add_url_rule('/crypto', 'crypto_market', self.crypto_market)
        
        # 策略管理
        self.app.add_url_rule('/strategy/smart_money', 'strategy_smart_money', self.strategy_smart_money)
        self.app.add_url_rule('/strategy/parameters', 'strategy_parameters', self.strategy_parameters)
        self.app.add_url_rule('/strategy/custom', 'strategy_custom', self.strategy_custom)
        
        # 回测系统
        self.app.add_url_rule('/backtest', 'backtest', self.backtest)
        self.app.add_url_rule('/backtest/new', 'backtest_new', self.backtest_new)
        self.app.add_url_rule('/backtest/history', 'backtest_history', self.backtest_history)
        self.app.add_url_rule('/backtest/optimize', 'backtest_optimize', self.backtest_optimize)
        
        # 交易系统
        self.app.add_url_rule('/trading', 'trading', self.trading)
        self.app.add_url_rule('/trading/execute', 'trading_execute', self.trading_execute)
        self.app.add_url_rule('/trading/positions', 'trading_positions', self.trading_positions)
        self.app.add_url_rule('/trading/history', 'trading_history', self.trading_history)
        
        # 系统设置
        self.app.add_url_rule('/settings', 'settings', self.settings)
        self.app.add_url_rule('/settings/index', 'settings_index', self.settings_index)
        self.app.add_url_rule('/settings/api', 'settings_api', self.settings_api)
        self.app.add_url_rule('/settings/system', 'settings_system', self.settings_system)
        self.app.add_url_rule('/settings/risk', 'settings_risk', self.settings_risk)
        self.app.add_url_rule('/settings/backtest', 'backtest_settings', self.backtest_settings)
        self.app.add_url_rule('/settings/trading', 'trading_settings', self.trading_settings)
        self.app.add_url_rule('/settings/notifications', 'notification_settings', self.notification_settings)
    ...
    def api_config(self):
        ...
        try:
            # 获取当前API配置
            crypto_api = self.api_config_manager.get_crypto_api_config()
            stock_api = self.api_config_manager.get_stock_api_config()
            ...
            logger.error(f"渲染API配置页面出错: {e}")
            self._log_system_event(f"渲染API配置页面出错: {e}", "error")
    ...
    # 新增策略管理路由
    def strategy_smart_money(self):
        """庄家行为识别策略"""
        return render_template('strategy/smart_money.html')
        
    def strategy_parameters(self):
        """策略参数设置"""
        return render_template('strategy/parameters.html')
        
    def strategy_custom(self):
        """自定义策略"""
        return render_template('strategy/custom.html')
    
    # 新增回测系统路由
    def backtest_new(self):
        """新建回测"""
    ...
    def backtest_optimize(self):
        """参数优化"""
        return render_template('backtest/optimize.html')
    
    # 新增交易系统路由
    def trading_execute(self):
        """交易执行"""
        return render_template('trading/execute.html')
        
    def trading_positions(self):
        """持仓管理"""
        return render_template('trading/positions.html')
        
    def trading_history(self):
        """交易历史"""
        return render_template('trading/history.html')
    
    # 新增系统设置路由
    def settings_risk(self):
        """风控设置"""
        return render_template('settings/risk.html')
    ...
    def test_template(self):
        ...
        print(f"模板文件列表: {os.listdir(template_dir)}")
        print(f"base.html内容: {open(os.path.join(template_dir, 'base.html')).read()[:100]}")
        print(f"navbar.html内容: {open(os.path.join(template_dir, 'navbar.html')).read()[:100]}")
    ...
    def settings_api(self):
        ...
        try:
            ...
            logger.error(f"加载API配置页面错误: {str(e)}")
            return render_template('error.html', 
                                  error_code="500", 
                                  error_message="无法加载API配置页面", 
    ...
    def backtest_settings(self):
        """回测设置页面"""
        return render_template('settings/backtest.html', config=self.config)
    
    def trading_settings(self):
        """交易设置页面"""
        return render_template('settings/trading.html', config=self.config)
            ...
            for file in os.listdir(logs_dir):
                if file.endswith('.log'):
                    ...
                    try:
                        ...
                        recent_logs = [f"无法读取日志: {str(e)}"]
                    
                    log_files.append({
                        'name': file,
                        'path': file_path,
                        'size': size,
                        'size_readable': self._get_readable_size(size),
                        'modified': modified,
