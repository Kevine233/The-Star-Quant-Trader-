...
class SmartMoneyDetector:
    ...
    def _validate_input_data(self, data: pd.DataFrame, required_cols: List[str]) -> None:
        """
        验证输入数据
        ...
        抛出:
            DataValidationError: 如果数据为空或缺少必需的列
        """
        if data.empty:
        ...
        if missing_cols:
            raise DataValidationError(f"输入数据缺少必要的列: {missing_cols}")
    ...
    def detect_volume_anomalies(self, data: pd.DataFrame, window: int = 20) -> pd.DataFrame:
        ...
        try:
            # 验证输入数据
            self._validate_input_data(data, ['volume'])
            ...
            try:
                ...
                raise CalculationError(f"计算成交量对数收益率失败: {e}")
            ...
            logger.warning(f"成交量异常检测数据验证失败: {e}")
            return data
        except CalculationError as e:
            logger.error(f"成交量异常检测计算失败: {e}")
            return data
        except Exception as e:
    ...
    def detect_price_manipulation(self, data: pd.DataFrame, window: int = None) -> pd.DataFrame:
        ...
        try:
            # 验证输入数据
            self._validate_input_data(data, ['open', 'high', 'low', 'close', 'volume'])
            ...
            try:
                ...
                raise CalculationError(f"计算价格波动率失败: {e}")
            ...
            try:
                ...
                logger.warning(f"计算价格-成交量相关性失败: {e}")
                # 继续执行，不中断整个流程
            ...
            try:
                ...
                raise CalculationError(f"计算价格操纵评分失败: {e}")
            ...
            logger.warning(f"价格操纵检测数据验证失败: {e}")
            return data
        except CalculationError as e:
            logger.error(f"价格操纵检测计算失败: {e}")
            return data
        except Exception as e:
    ...
    def get_params(self):
        ...
        return {
            ...
            'price_manipulation_window': self.price_manipulation_window,
            'big_order_threshold': self.big_order_threshold,
